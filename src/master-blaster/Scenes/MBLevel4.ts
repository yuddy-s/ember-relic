...
export default class Level4 extends MBLevel {
    ...
    protected updateBossGate(): void {
        if(this.bossGateLayer === null || this.bossGateCleared || this.player === undefined){
            return;
        }

        if(!this.bossGateActive && this.player.position.x >= Level4.BOSS_GATE_TRIGGER_X){
            this.bossGateActive = true;
            this.bossGateLayer.visible = true;
            this.bossGateLayer.enablePhysics();
        }

        if(this.bossGateActive && this.boss !== undefined && this.boss.isDefeated()){
            this.clearBossGate();
        }
    }
    ...
    protected clearBossGate(): void {
        ...
        for(let y = 0; y < dims.y; y++){
            for(let x = 0; x < dims.x; x++){
                this.bossGateLayer.setTileAtRowCol(new Vec2(x, y), 0);
            }
        }
